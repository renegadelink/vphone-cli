...
`make setup_tools` 会安装所需主机工具，按需安装 `git-lfs`，并构建 vendored 的 `inject` 与 `ldid`。
...
```bash
make setup_tools              # 安装所需主机工具并构建 vendored inject/ldid
...
```bash
# 终端 2
make restore_get_shsh         # 通过 MobileDevice.framework 请求个性化 SHSH 数据
make restore                  # 通过 MobileDevice.framework 刷写固件
```
...
当 ramdisk 运行后（输出中应显示 `Running server`），打开**第三个终端**运行 USBMux 转发器，然后在终端 2 安装 CFW：
