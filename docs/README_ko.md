...
`make setup_tools`는 필요한 호스트 도구를 설치하고, 필요 시 `git-lfs`를 설치하며, vendored `inject`와 `ldid`를 빌드합니다.
...
램디스크가 실행되면(출력에 `Running server`가 표시됨), **세 번째 터미널**을 열어 USBMux 포워더를 시작한 후, 터미널 2에서 커스텀 펌웨어를 설치합니다:
